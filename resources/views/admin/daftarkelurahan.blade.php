...
<div class="min-h-screen bg-gray-50 p-7">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-lg font-medium text-gray-800">Daftar Kelurahan</h1>
      <p class="text-sm text-gray-500 mt-0.5">Total {{ $kelurahans->count() }} kelurahan terdaftar</p>
    </div>
      ...
      <table class="w-full text-sm text-left">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-200 text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            <th class="px-4 py-3">Nama Kelurahan</th>
        ...
        <tbody class="divide-y divide-gray-100">
          @forelse ($kelurahans as $index => $kel)
            <tr class="hover:bg-gray-50 transition">
              ...
              <td class="px-4 py-3 font-medium text-gray-800">{{ $kel->nama_kelurahan }}</td>
              <td class="px-4 py-3">
                <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-teal-50 text-teal-700 text-xs font-medium border border-teal-100">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-2">
                  ...
                  {{-- Edit --}}
                  <a href="{{ route('admin.kelurahan.edit', $kel->id_kelurahan) }}"
                  ...
                  {{-- Hapus --}}
                  <form action="{{ route('admin.kelurahan.destroy', $kel->id_kelurahan) }}" method="POST"
                    onsubmit="return confirm('Yakin ingin menghapus kelurahan {{ $kel->nama_kelurahan }}?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
